...
namespace Finbuckle.MultiTenant.Contrib.Identity
{
    ...
    public class MultiTenantUserClaimsPrincipalFactory<TUser, TUserRole> : UserClaimsPrincipalFactory<TUser, TUserRole> where TUser : class where TUserRole : class
    {
        ...
        protected override async Task<ClaimsIdentity> GenerateClaimsAsync(TUser user)
        {
            ...
            // IdentityServer4 UserClaimsFactory will remove/replace the
            // default identity name claim that is auto generated by the UserClaimsPrincipalFactory.
            // Therefore, to have consistency we will remove it here.
            var username = await UserManager.GetUserNameAsync(user);
            var usernameClaim = id.FindFirst(claim => 
                claim.Type == UserManager.Options.ClaimsIdentity.UserNameClaimType 
                && claim.Value == username);
           
            if (usernameClaim != null)
            {
                id.RemoveClaim(usernameClaim);
